 ...
 $Email = $_POST['email'];
 $busca_query = mysql_query("SELECT cont_email, cont_senha FROM usu_contribuidor WHERE cont_email = '$Email' ")or die(mysql_error());
  if (mysql_num_rows($busca_query) == 0) { //Se nao achar nada, lança essa mensagem
    echo file_get_contents('esquecisenhacontribuidor.html');
  ...
  if ($dados = mysql_fetch_array($busca_query)) {
    if(($dados['cont_email'] != NULL)){
     $Vai = "Sua senha de email para a BDMPEC é: $dados[cont_senha].";
